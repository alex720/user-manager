...
struct TS3Functions {
	...
	anyID getClientIdByUniqueId(const char* cluid, uint64 serverConnectionHandlerID) {
		...
		getClientList(serverConnectionHandlerID, &clients);
		char* uid;

		for (int i = 0; clients[i]; ++i) {
			if (getClientVariableAsString(serverConnectionHandlerID, clients[i], CLIENT_UNIQUE_IDENTIFIER, &uid) == ERROR_ok) {
				if (!strcmp(cluid, uid))
					foundid = clients[i];
	...
	void setchannelgroup(uint64 serverConnectionHandlerID, anyID clientID, uint64 ChannelID,int channelgroup) {
		uint64 DBID;
		getClientVariableAsUInt64(serverConnectionHandlerID, clientID, 0x20, &DBID);
		//DBID= ts3Functions.requestClientDBIDfromUID(serverConnectionHandlerID, UID,nullptr/*returnCode*/);
		...
		requestSetClientChannelGroup(serverConnectionHandlerID, channelGroupIDs, channelIDs, userIDs,
